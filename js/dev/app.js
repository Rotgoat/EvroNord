...
function formInit() {
  ...
  document.querySelectorAll('input[type="file"].input-file').forEach(function(input) {
    const button = document.querySelector(".form__add-file-btn");
    if (button) {
      button.addEventListener("click", function(e) {
        if (!e.target.classList.contains("file-clear") && !e.target.closest(".file-clear")) {
          e.preventDefault();
          input.click();
        }
      });
    ...
    if (nextElement) {
      nextElement.setAttribute("data-original-text", nextElement.innerHTML);
    }
    function resetFileInput() {
      input.value = "";
      let nextElement2 = input.nextElementSibling;
      if (nextElement2) {
        nextElement2.innerHTML = nextElement2.getAttribute("data-original-text");
        nextElement2.classList.remove("_upload");
        let cross = nextElement2.querySelector(".file-clear");
        if (cross) {
          cross.remove();
        }
      }
    ...
    input.addEventListener("change", function() {
      ...
      let nextElement2 = this.nextElementSibling;
      if (nextElement2) {
        if (file) {
          let cross = document.createElement("span");
          cross.className = "file-clear";
          cross.innerHTML = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.92984 17.0718C1.97473 16.1493 1.21291 15.0459 0.688821 13.8258C0.164731 12.6058 -0.111131 11.2936 -0.122669 9.9658C-0.134207 8.63801 0.11881 7.32121 0.621618 6.09225C1.12443 4.86328 1.86696 3.74677 2.80589 2.80784C3.74481 1.86891 4.86133 1.12638 6.09029 0.623572C7.31926 0.120763 8.63605 -0.132254 9.96385 -0.120716C11.2916 -0.109178 12.6038 0.166684 13.8239 0.690774C15.0439 1.21486 16.1474 1.97669 17.0698 2.93179C18.8914 4.81781 19.8994 7.34383 19.8766 9.9658C19.8538 12.5878 18.8021 15.0959 16.948 16.95C15.0939 18.8041 12.5858 19.8557 9.96385 19.8785C7.34188 19.9013 4.81586 18.8934 2.92984 17.0718ZM11.3998 10.0018L14.2298 7.17179L12.8198 5.76179L9.99984 8.59179L7.16984 5.76179L5.75984 7.17179L8.58984 10.0018L5.75984 12.8318L7.16984 14.2418L9.99984 11.4118L12.8298 14.2418L14.2398 12.8318L11.4098 10.0018H11.3998Z" fill="#919395"/></svg>';
          cross.title = "Удалить файл";
          cross.addEventListener("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            resetFileInput();
          });
          let oldCross = nextElement2.querySelector(".file-clear");
          if (oldCross) {
            oldCross.remove();
          }
          nextElement2.innerHTML = file.name;
          nextElement2.appendChild(cross);
          nextElement2.classList.add("_upload");
        } else {
          nextElement2.innerHTML = nextElement2.getAttribute("data-original-text");
          nextElement2.classList.remove("_upload");
        }
...
function rangeInit() {
  ...
  if (priceSlider) {
    priceSlider.getAttribute("data-fls-range-from");
    priceSlider.getAttribute("data-fls-range-to");
...
function menuInit() {
  document.addEventListener("click", function(e) {
    if (bodyLockStatus && e.target.closest("[data-fls-filter]")) {
      bodyLockToggle();
      document.documentElement.toggleAttribute("data-fls-filter-open");
    }
    if (bodyLockStatus && e.target.closest("[data-fls-filter-close]")) {
      bodyLockToggle();
      document.documentElement.toggleAttribute("data-fls-filter-open");
    }
  });
...
function initSliders() {
  ...
  if (document.querySelector(".slider-newscard")) {
    new Swiper(".slider-newscard", {
      // 
      modules: [Navigation],
      observer: true,
      observeParents: true,
  ...
  if (document.querySelector(".portfoliocard__slider")) {
    const portfolioCardSlider = new Swiper(".portfoliocard__slider-thumbs", {
    ...
    new Swiper(".portfoliocard__slider-head", {
      modules: [Navigation, Thumb],
      slidesPerView: 1,
      speed: 500,
      loop: true,
  ...
  if (document.querySelector(".tabs-slider")) {
    ...
    let isSwiping = false;
    tabsSLider.on("touchStart", function() {
      isSwiping = false;
    });
    tabsSLider.on("touchMove", function() {
      isSwiping = true;
    });
    tabsSLider.on("click", function(e) {
      if (!isSwiping && tabsSLider.clickedIndex !== void 0) {
        tabsSLider.slideTo(tabsSLider.clickedIndex, 300);
      }
    });
    tabsSLider.on("touchEnd", function() {
      setTimeout(() => {
        isSwiping = false;
      }, 100);
    });
  ...
  if (document.querySelector(".slider-product")) {
    const portfolioCardSlider = new Swiper(".product-thumb-slider", {
      // <- Вказуємо склас потрібного слайдера
      modules: [Navigation, Thumb],
    ...
    new Swiper(".product-head-slider", {
      modules: [Navigation, Thumb],
      slidesPerView: 1,
      speed: 500,
      watchSlidesProgress: true,
...
function initGallery() {
  if (document.querySelector("[data-fls-gallery]")) {
    new lightGallery(document.querySelector("[data-fls-gallery].licenses-gallery"), {
      // plugins: [lgZoom],
      licenseKey: KEY,
      selector: "a",
      speed: 500,
